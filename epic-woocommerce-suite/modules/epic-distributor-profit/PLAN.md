...
- ~~Product Cost master / GIÁ VỐN breakdown screen and auto-fill.~~ **Built 2026-09-12** as a separate plugin, `epic-product-cost` — see its own PLAN.md. This plugin's order-picker now calls `Epic_Product_Cost::estimate_order_cost()` when that plugin is active (optional, no hard dependency) to auto-fill the Cost field on "Import from Order"; manual entries are unaffected.

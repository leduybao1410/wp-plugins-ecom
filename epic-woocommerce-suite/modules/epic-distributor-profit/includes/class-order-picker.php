...
class Epic_Distributor_Profit_Order_Picker {
	...
	private static function format_order( $order ) {
		...
		// EPIC Product Cost is an optional companion plugin (no hard dependency either way) — if
		// it's active, ask it for a cost estimate so admin doesn't have to type one in from scratch.
		// Estimated as of the order's own date, so a later cost change never rewrites an old order's
		// estimate. Still just a starting point: the Cost field on the entry form stays editable.
		$cost           = '';
		$cost_complete  = true;
		if ( class_exists( 'Epic_Product_Cost' ) ) {
			$estimate = Epic_Product_Cost::estimate_order_cost( $order );
			if ( $estimate ) {
				$cost          = wc_format_decimal( $estimate['total'], 2 );
				$cost_complete = ! empty( $estimate['complete'] );
			}
		}

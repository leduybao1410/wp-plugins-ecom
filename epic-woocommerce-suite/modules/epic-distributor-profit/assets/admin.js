...
( function ( $ ) {
	...
	function fillFromOrder( order ) {
		...
		// Auto-filled by EPIC Product Cost when that plugin is active; still a plain editable
		// number afterwards. An empty/undefined order.cost (plugin not active, or nothing known
		// yet for these products) leaves the Cost field exactly as before — manual entry.
		var $costHint = $( '#epic-dp-cost-hint' );
		if ( order.cost ) {
			$( '#cost' ).val( order.cost );
			if ( $costHint.length ) {
				$costHint.text(
					order.cost_complete
						? EpicDistributorProfit.i18n.costEstimated
						: EpicDistributorProfit.i18n.costEstimatedPartial
				).show();
			}
		} else if ( $costHint.length ) {
			$costHint.hide();
		}
	...
	function renderResults( orders ) {
		...
		orders.forEach( function ( order ) {
			...
			if ( order.cost ) {
				label += ' [' + EpicDistributorProfit.i18n.estCostLabel + ': ' + order.cost + ']';
			}

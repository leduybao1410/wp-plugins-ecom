...
class Epic_Wholesale_Orders_Store {
	...
	const USER_META_VIP       = 'epic_wholesale_vip';
	...
	/**
	 * VIP customers see NO prices on the storefront wholesale page; the order
	 * is recorded without prices and the seller sets a final price per item on
	 * the order's edit screen afterwards (see class-meta-box.php). A VIP order
	 * cannot be Approved until every line item is priced.
	 *
	 * @param int|string $user_id
	 * @return bool
	 */
	public static function is_vip( $user_id ) {
		return 'yes' === get_user_meta( (int) $user_id, self::USER_META_VIP, true );
	}
	 ...
	 * @return bool
	 */
	public static function set_vip( $user_id, $is_vip ) {
		return update_user_meta( (int) $user_id, self::USER_META_VIP, $is_vip ? 'yes' : 'no' );
	}

	/**
	 * True when every line item carries a unit price (all-or-nothing). Used for
	 * VIP orders: an order is only "priced" once the seller has set a price for
	 * EVERY item — that is what unlocks approval. Non-VIP orders are always
	 * priced (their prices are computed at submission).
	 *
	 * @param array $items
	 * @return bool
	 */
	public static function items_priced( array $items ) {
		if ( empty( $items ) ) {
			return false;
		}
		foreach ( $items as $item ) {
			$unit = isset( $item['unit_price'] ) ? (string) $item['unit_price'] : '';
	 ...
	 *                                 VIP items carry '' for unit_price/line_total
	 *                                 until the seller prices them.
	 ...
	 * @param bool   $is_vip           True for a VIP order: prices are set by the
	 *                                 seller after submission, not at creation.
	 * @return int Post id, or 0 on failure.
	 */
	public static function create_order( $customer_user_id, array $items, $note = '', $customer_name = '', $customer_email = '', $level_key = '', $level_name = '', $level_discount = 0, $is_vip = false ) {
		$total = 0.0;
		foreach ( $items as $item ) {
		...
		// An unpriced VIP order stores an empty total — the metabox recomputes
		// and writes it once the seller prices every line item.
		update_post_meta( $post_id, self::META_TOTAL, self::items_priced( $items ) ? (string) $total : '' );
		update_post_meta( $post_id, self::META_IS_VIP, $is_vip ? 'yes' : 'no' );
	...
	public static function get_order( $post_id ) {
		...
		$is_vip = 'yes' === get_post_meta( $post_id, self::META_IS_VIP, true );
		$priced = $is_vip ? self::items_priced( $items ) : true;
		...
		return array(
			...
			// Null until a VIP order is priced — non-VIP orders always carry a total.
			'total'          => $priced ? (float) get_post_meta( $post_id, self::META_TOTAL, true ) : null,

...
class Epic_Wholesale_Orders_List_Table extends WP_List_Table {
	...
	protected function column_level( $item ) {
		if ( ! empty( $item['is_vip'] ) ) {
			return '<span style="color:#b45309; font-weight:600;">' . esc_html__( 'VIP', 'epic-wholesale-orders' ) . '</span>';
		}
	...
	protected function column_total( $item ) {
		// Unpriced VIP orders carry no total yet.
		if ( ! empty( $item['is_vip'] ) && ( null === $item['total'] || '' === $item['total'] ) ) {
			return '<span style="color:#999;">' . esc_html__( 'Not set', 'epic-wholesale-orders' ) . '</span>';
		}

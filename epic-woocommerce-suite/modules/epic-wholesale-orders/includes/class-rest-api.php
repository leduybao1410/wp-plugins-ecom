...
class Epic_Wholesale_Orders_Rest_Api {
	...
	public static function list_products( \WP_REST_Request $request ) {
		...
		// wholesale price discounted by this level's percentage. VIP customers
		// see NO prices at all (the seller sets a final per-item price on each
		// order later), so the level is returned for bookkeeping only.
		...
		$is_vip = Epic_Wholesale_Orders_Store::is_vip( $customer->ID );
		...
		foreach ( $simple->posts as $post ) {
			...
			$entry = self::product_entry( $product, $level_key, $is_vip );
			if ( $entry ) {
				$products[] = $entry;
			}
		}
		...
		foreach ( $variations->posts as $post ) {
			...
			$entry = self::variation_entry( $variation, $level_key, $is_vip );
			if ( $entry ) {
				$products[] = $entry;
			}
		}
	 ...
	 * @param bool        $hide_prices True for VIP customers — include the
	 *                                 product but expose no prices (seller
	 *                                 prices it per order instead).
	 * @return array|null Null when the wholesale price is missing/zero (only
	 *                    for non-VIP customers).
	 */
	private static function product_entry( \WC_Product $product, $level_key, $hide_prices = false ) {
		if ( $hide_prices ) {
			return array(
				'id'                   => $product->get_id(),
				'parent_id'            => 0,
	 ...
	 * @param string                $level_key
	 * @param bool                  $hide_prices
	 * @return array|null
	 */
	private static function variation_entry( \WC_Product_Variation $variation, $level_key, $hide_prices = false ) {
		if ( $hide_prices ) {
			return array(
				'id'                   => $variation->get_id(),
	...
	public static function create_order( \WP_REST_Request $request ) {
		...
		// the level is snapshotted onto the order for the record. VIP orders
		// ignore it entirely: the seller sets a final price per item later.
		$level_key = Epic_Wholesale_Orders_Store::get_customer_level( $customer->ID );
		$level     = Epic_Wholesale_Orders_Store::get_level( $level_key );
		if ( ! $level ) {
			$level_key = Epic_Wholesale_Orders_Store::get_default_level_key();
		...
		// Recompute everything server-side — never trust client prices. For a
		// VIP order no price exists yet: line items are recorded with empty
		// unit_price/line_total and the seller prices them on the edit screen.
		$items = array();
		$total = 0.0;
		foreach ( $raw_items as $raw ) {
			...
			if ( '' === $base_price || (float) $base_price <= 0 ) {
				return new \WP_Error( 'epic_wholesale_orders_not_eligible', sprintf( 'Product %d is not available for wholesale ordering.', $product_id ), array( 'status' => 400 ) );
			}

...
class Epic_Wholesale_Orders_Settings {
	...
	public static function handle_save_customers() {
		...
		// VIP flags: a VIP customer sees no prices on the storefront wholesale
		// page; the seller sets a final per-item price on each order afterwards.
		$raw_vip = isset( $_POST['customer_vip'] ) && is_array( $_POST['customer_vip'] ) ? wp_unslash( $_POST['customer_vip'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized via Store::set_vip().
		foreach ( $whitelist as $user_id ) {
			$is_vip = isset( $raw_vip[ $user_id ] ) && 'yes' === sanitize_key( (string) $raw_vip[ $user_id ] );
			Epic_Wholesale_Orders_Store::set_vip( $user_id, $is_vip );
		}
					...
					<tr>
						...
						<td>
							...
							<?php else : ?>
								<table class="widefat striped" style="max-width:720px;">
									<thead>
										<tr>
											...
											<th style="width:90px;"><?php esc_html_e( 'VIP', 'epic-wholesale-orders' ); ?></th>
										</tr>
									</thead>
									<tbody>
										<?php foreach ( $customers as $user_id ) : ?>
											...
											<tr>
												...
												<td>
													<label title="<?php esc_attr_e( 'VIP customers place orders without seeing prices; the seller sets a final price per item on each order afterwards.', 'epic-wholesale-orders' ); ?>">
														<input
															type="checkbox"
															name="customer_vip[<?php echo esc_attr( $user_id ); ?>]"
															value="yes"
															<?php checked( Epic_Wholesale_Orders_Store::is_vip( $user_id ) ); ?>
														/>
														<?php esc_html_e( 'VIP', 'epic-wholesale-orders' ); ?>
													</label>
												</td>
											</tr>
										<?php endforeach; ?>
								...
								<p class="description">
									<?php esc_html_e( 'VIP customers order without seeing prices — the storefront hides all prices for them. The seller sets a final price for every item on each order\'s edit screen, and a VIP order cannot be Approved until every item is priced. The price the seller sets is final (no level discount is applied).', 'epic-wholesale-orders' ); ?>
								</p>
							<?php endif; ?>
						</td>
					</tr>
						...
						<td>
							...
							<p class="description">
								...
								<?php if ( $epic_secret_is_set ) : ?>
									<br />
									<?php esc_html_e( 'A secret is saved. Leave this field blank to keep it, or paste a new value to replace it.', 'epic-wholesale-orders' ); ?>
								<?php endif; ?>
							</p>
						</td>
					</tr>

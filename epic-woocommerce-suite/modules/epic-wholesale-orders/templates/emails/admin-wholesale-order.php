 ...
 * @var bool     $is_vip     True for a VIP order (recorded without prices).
 * @var bool     $priced     True once the seller has priced every line item.
...
// VIP orders arrive unpriced — show "—" for their prices until set.
...
<?php if ( $is_vip && ! $priced ) : ?>
	<p style="padding:10px 12px; background:#fdf6ec; border:1px solid #f0d9a8;">
		<strong><?php esc_html_e( 'Đơn hàng VIP — khách hàng không thấy giá.', 'epic-wholesale-orders' ); ?></strong><br/>
		<?php esc_html_e( 'Vui lòng định giá từng sản phẩm trong WooCommerce → Wholesale Orders trước khi duyệt đơn. Giá bạn nhập là giá cuối cùng (không áp dụng chiết khấu theo mức giá).', 'epic-wholesale-orders' ); ?>
	</p>
<?php endif; ?>
			...
			<tr>
				...
				<td style="padding:12px; text-align:right;">
					...
					if ( isset( $item['unit_price'] ) && '' !== $item['unit_price'] && (float) $item['unit_price'] > 0 ) {
						echo wp_kses_post( wc_price( $item['unit_price'] ) );
					} else {
						echo '&#8212;';
					}
					?>
				...
				<td style="padding:12px; text-align:right;">
					...
					if ( isset( $item['line_total'] ) && '' !== $item['line_total'] && is_numeric( $item['line_total'] ) ) {
						echo wp_kses_post( wc_price( $item['line_total'] ) );
					} else {
						echo '&#8212;';
					}
					?>
